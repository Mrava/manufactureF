...
function add()
{
	var url = "/makePlan!add.action";
	self.location = url;
}
function edit(id)
{
	var url = "/makePlan!edit.action?pojo.id="+id;
	self.location = url;
}
	
function del(id)
{	
	var url = "/makePlan!delete.action?pojo.id="+id;
	if (confirm("是否要删除?"))
		self.location = url;
}
...
<div class="listcon">
	<div class="gopage" style="margin-right:10px;margin-bottom:5px;">
		...
		<b onclick="gotopage_up('makePlan',<s:text name='paper.max' />,<s:text name='paper.count' />,<s:text name="paper.size" />)" class="btn_gopage" >G0</b>
	...
	<s:iterator value="dataList" id="makeplan" status="it">
		<tr>
	...
	<div class="gopage" style="margin-right:10px;">
		...
		onpropertychange='if(!/\D+/.test(this.value)){return;};this.value=this.value.replace(/\D+/g, "")'
		onblur = 'this.value = this.value.replace(/\D+/g, "")' />
		<b onclick="gotopage('makePlan',<s:text name='paper.max' />,<s:text name='paper.count' />,<s:text name="paper.size" />)" class="btn_gopage" >G0</b>
		<span>总数：<s:text name='paper.count' /> ，共 <s:text name="paper.max" /> 页</span>
	</div>
</body>
</html>
