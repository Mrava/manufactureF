...
function add()
{
	var url = "/materialPlan!add.action";
	self.location = url;
}
function addBatch()
{			   
	var url = "/materialPlan!addBatch.action";
	self.location = url;
}
function edit(id)
{
	var url = "/materialPlan!edit.action?pojo.id="+id;
	self.location = url;
}
	
function del(id)
{	
	var url = "/materialPlan!delete.action?pojo.id="+id;
	if (confirm("是否要删除?"))
		self.location = url;
}
...
<div class="listtit">
	...
	<a href="javascript:addBatch()" style="margin-right:10px;">批量添加</a>
</div>
...
<div class="listcon">
	<div class="gopage" style="margin-right:10px;margin-bottom:5px;">
		...
		<b onclick="gotopage_up('materialPlan',<s:text name='paper.max' />,<s:text name='paper.count' />,<s:text name="paper.size" />)" class="btn_gopage" >G0</b>
		<span>总数：<s:text name='paper.count' /> ，共 <s:text name="paper.max" /> 页</span>
	</div>
	<table cellpadding="1" cellspacing="1" bgcolor="#cbcbcb">
	<tr>
		<td class="titletd"></td>
		<td class="titletd">BOM编号</td>
		<td class="titletd">BOM代码</td>
		<td class="titletd">BOM名称</td>
		<td class="titletd">BOM类型</td>
		<td class="titletd">数量</td>
		<td class="titletd">日期</td>
		<td class="titletd"></td>
	</tr>
	<s:iterator value="dataList" id="materialPlan" status="it">
		<tr>
	...
	<div class="gopage" style="margin-right:10px;">
		...
		onpropertychange='if(!/\D+/.test(this.value)){return;};this.value=this.value.replace(/\D+/g, "")'
		onblur = 'this.value = this.value.replace(/\D+/g, "")' />
		<b onclick="gotopage('materialPlan',<s:text name='paper.max' />,<s:text name='paper.count' />,<s:text name="paper.size" />)" class="btn_gopage" >G0</b>
		<span>总数：<s:text name='paper.count' /> ，共 <s:text name="paper.max" /> 页</span>
	</div>
</div>
</body>
