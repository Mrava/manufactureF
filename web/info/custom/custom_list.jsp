...
function add()
{
	var url = "/custom!add.action";
	self.location = url;
}

function edit(id)
{
	var url = "/custom!edit.action?pojo.id="+id;
	self.location = url;
}
	
function del(id)
{
	var url = "/custom!delete.action?pojo.id="+id;
	self.location = url;
}
function load()
{
	var method="<s:text name='message.message' />";
...
function find()
{
	...
	var url = "/custom!search.action?searchContext="+searchText;
	self.location = url;			
}
...
<div class="listcon">
	...
	<div class="gopage" style="margin-right:10px;margin-bottom:5px;">
		...
		<b onclick="gotopage_up('custom',<s:text name='paper.max' />,<s:text name='paper.count' />,<s:text name="paper.size" />)" class="btn_gopage" >G0</b>
	...
	<div class="gopage" style="margin-right:10px;">
		...
		onpropertychange='if(!/\D+/.test(this.value)){return;};this.value=this.value.replace(/\D+/g, "")'
		onblur = 'this.value = this.value.replace(/\D+/g, "")' />
		<b onclick="gotopage('custom',<s:text name='paper.max' />,<s:text name='paper.count' />,<s:text name="paper.size" />)" class="btn_gopage" >G0</b>
		<span>总数：<s:text name='paper.count' /> ，共 <s:text name="paper.max" /> 页</span>
	</div>
</div>
</body>
