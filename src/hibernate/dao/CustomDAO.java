...
public class CustomDAO extends HibernateDaoSupport
{
	...
	public Custom findById(java.lang.Integer id)
	{
		...
		try
		{
			Custom instance = (Custom) getHibernateTemplate().get("hibernate.dao.Custom", id);
			return instance;
		} catch (RuntimeException re)
		{
	...
	{
		log.debug("finding Custom instance with property: " + propertyName + ", value: " + value);
		try
		{
			String queryString = "from Custom as model where model." + propertyName + "= ?";
	...
	public Custom merge(Custom detachedInstance)
	{
		...
		try
		{
			Custom result = (Custom) getHibernateTemplate().merge(detachedInstance);
